...
def index(request):
    detail = '   '  # Default value in case of GET request or any failure
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Check if live_price has at least 2 elements
            if len(live_price) > 1:
                live_bitcoin_price = live_price[1].getText()
                live_bitcoin_price1 = live_price[1].getText()
            else:
                live_bitcoin_price = 'N/A'
                live_bitcoin_price1 = 'N/A'
            
            res = requests.get(f'https://www.blockchain.com/btc/address/{addr}')
            if res:
                soup = bs4.BeautifulSoup(res.text, 'lxml')
                bal = soup.find_all('span', {'class': 'sc-1ryi78w-0 gCzMgE sc-16b9dsl-1 kUAhZx u3ufsr-0 fGQJzg'})
                
                # Ensure there are enough balance elements
                if len(bal) >= 5:
                    ...
                    raise ValueError("Balance elements not found")
